"""
F1 Pro Dashboard  v4.8 (OPTIMIZADO)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
class F1Dashboard:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        # --- Variables añadidas para la Optimización (Option A) ---
        self._dot_ids: Dict[str, Dict[str, int]] = {}
        self._plot_last_lap: int = -1
        self._plot_last_drv: str = ""
        self._plot_last_w: int = 0
        # ----------------------------------------------------------
    ...
    def _draw_track_map(self) -> None:
        ...
        self._dot_ids = {} # Limpiamos el pool de objetos al cargar nuevo mapa
        try:
    ...
    def _redraw_track(self) -> None:
        ...
        # NOTA: NO borramos los puntos de los coches ("car_dot") aquí para mantener el object pool intacto
        self.map_canvas.create_polygon(
            self._map_pts_cache, outline="#444466", fill="", width=4, tags="track")
        # Aseguramos que la pista siempre quede por debajo de los coches
        self.map_canvas.tag_lower("track")
    ...
    # ---> OPTIMIZACIÓN 1: Object Pooling en el mapa
    def _update_map_dots(self, snaps: List[Snap]) -> None:
        for s in snaps:
            # Si el coche está fuera o no hay coordenadas, lo ocultamos
            if s.is_out or math.isnan(s.x) or math.isnan(s.y): 
                if s.drv in self._dot_ids:
                    self.map_canvas.itemconfigure(self._dot_ids[s.drv]['oval'], state='hidden')
                    self.map_canvas.itemconfigure(self._dot_ids[s.drv]['text'], state='hidden')
                continue
            ...
            if s.drv not in self._dot_ids:
                # Creación inicial (solo ocurre una vez por piloto al cargarse en pista)
                o_id = self.map_canvas.create_oval(cx-size, cy-size, cx+size, cy+size, 
                                                   fill=col, outline="#ffffff" if sel else col, 
                                                   width=2 if sel else 1, tags="car_dot")
                t_id = self.map_canvas.create_text(cx, cy-size-8, text=s.abbr, 
                                                   fill=col, font=(FONT, 8, "bold"), tags="car_dot")
                self._dot_ids[s.drv] = {'oval': o_id, 'text': t_id}
            else:
                # Update In-Place (súper rápido, no destruye objetos de tk)
                o_id = self._dot_ids[s.drv]['oval']
                t_id = self._dot_ids[s.drv]['text']
                
                self.map_canvas.coords(o_id, cx-size, cy-size, cx+size, cy+size)
                self.map_canvas.itemconfigure(o_id, outline="#ffffff" if sel else col, 
                                              width=2 if sel else 1, state='normal')
                
                self.map_canvas.coords(t_id, cx, cy-size-8)
                self.map_canvas.itemconfigure(t_id, state='normal' if sel else 'hidden')
    ...
    # ---> OPTIMIZACIÓN 2: In-place update en Treeview de análisis
    def _update_analysis(self, snap: Snap, drv: str, abs_t: float) -> None:
        ...
        cur_dict  = {col: drv_laps[col].to_numpy()[idx_nxt] for col in drv_laps.columns}
        prev_dict = ({col: drv_laps[col].to_numpy()[idx_nxt-1] for col in drv_laps.columns}
                     if idx_nxt > 0 else {})
        ...
        if lap_changed or drv_changed:
            ...
            # Update de Sectores In-Place
            for s_idx in range(1, 4):
                col  = f"Sector{s_idx}Time"
                cur_s  = 0.; prev_s = 0.
                if col in drv_laps.columns:
                    v = cur_dict.get(col, float('nan'))
                    if v is not None and not math.isnan(float(v)): cur_s = float(v)
                    if prev_dict:
                        pv = prev_dict.get(col, float('nan'))
                        if pv is not None and not math.isnan(float(pv)): prev_s = float(pv)
                ...
                sec_cond = "1"
                if lap_start_abs is not None and self.track_status_df is not None and not self.track_status_df.empty and len(self._ts_times) > 0:
                ...
                elif cur_s and self._overall_sector_best[s_idx-1] and abs(cur_s - self._overall_sector_best[s_idx-1]) < 0.05:
                    row_tag = "f1_purple"
                ...
                item_id = f"S{s_idx}"
                vals = (item_id, f"{cur_s:.3f}" if cur_s else "–", f"{diff:+.3f}" if diff else "–", verdict)
                if self.tree_sec.exists(item_id):
                    self.tree_sec.item(item_id, values=vals, tags=(row_tag,))
                else:
                    self.tree_sec.insert("", "end", iid=item_id, values=vals, tags=(row_tag,))
            ...
            avg_id = "AVG"
            vals_avg = ("AVG", avg_vals[0], avg_vals[1], avg_vals[2])
            if self.tree_sec.exists(avg_id):
                self.tree_sec.item(avg_id, values=vals_avg, tags=("avg_row",))
            else:
                self.tree_sec.insert("", "end", iid=avg_id, values=vals_avg, tags=("avg_row",))
        ...
        if not completed.empty:
            ...
            if len(valid):
                ...
                if len(valid) >= 2:
                    ...
                    diff = ll_s - avg
                    if diff < -0.5:
                        pace_txt = f"FLYING ↑↑ ({diff:+.2f}s)"; pace_col = F1_PURPLE
                    elif diff < 0:
                        pace_txt = f"IMPROVING ↑ ({diff:+.2f}s)"; pace_col = GREEN
                    elif diff < 0.3:
                        pace_txt = f"CONSISTENT ({diff:+.2f}s)"; pace_col = TEXT_PRI
                    else:
                        pace_txt = f"DROPPING ↓ ({diff:+.2f}s)"; pace_col = ACCENT

                if len(valid) >= 3:
                    trend = np.polyfit(range(len(valid[-5:])), valid[-5:], 1)[0]
                    if trend > 0.15: insights.append(f"⚠ Tyre deg +{trend:.2f}s/L")
                    elif trend > 0.05: insights.append(f"🔴 Tyre wearing ({trend:.2f}s/L)")

                if is_2026_era(self.session_year):
                    soc_now = snap.ers_soc
                    if snap.is_clipping: insights.append("⚡ SUPERCLIPPING — recharging")
                    elif soc_now < 15: insights.append(f"⚡ ERS critical ({soc_now:.0f}%)")
                    elif soc_now > 80: insights.append(f"🔋 ERS full ({soc_now:.0f}%) — save mode?")
    ...
    # ---> OPTIMIZACIÓN 3: Capas Estática vs Dinámica en el Plotter
    def _update_telemetry_plot(self, drv: str, abs_t: float) -> None:
        ...
        try:
            ...
            lt_arr   = drv_laps["LapTime"].to_numpy(dtype=float) if "LapTime" in drv_laps.columns else None
            lapnum_arr = drv_laps["LapNumber"].to_numpy(dtype=float) if "LapNumber" in drv_laps.columns else None
            ...
            # =========================================================
            # CAPA DINÁMICA: Se ejecuta en cada frame (muy ligero)
            # =========================================================
            def draw_dynamic():
                c.delete("_plot_cursor")
                PROG_Y = H - 4
                
                # Máscara negra interactiva que oculta el futuro del ERS
                if ERS_H > 0:
                    c.create_rectangle(cursor_x, LABEL_H, W, LABEL_H + ERS_H, fill="#03030a", outline="", tags="_plot_cursor")
                
                # Barra amarilla de progreso de vuelta y cursor vertical
                c.create_rectangle(0, PROG_Y, cursor_x, H, fill=YELLOW, outline="", tags="_plot_cursor")
                c.create_line(cursor_x, 0, cursor_x, PROG_Y, fill="#888800", width=5, tags="_plot_cursor")
                c.create_line(cursor_x, 0, cursor_x, PROG_Y, fill=YELLOW, width=2, tags="_plot_cursor")
                tri = [cursor_x - 8, 0, cursor_x + 8, 0, cursor_x, 12]
                c.create_polygon(tri, fill=YELLOW, outline="#000000", width=1, tags="_plot_cursor")

                # Texto de estado interactivo (WOT, BRK, L&C) extrayendo solo el frame actual del array completo
                si_dyn = int(np.searchsorted(t_vals, lap_abs_start, side='left'))
                ei_dyn = int(np.searchsorted(t_vals, lap_abs_end,   side='right'))
                n_dyn = ei_dyn - si_dyn
                if n_dyn > 0:
                    ci = min(max(int(cursor_prog * (n_dyn - 1)), 0), n_dyn - 1)
                    idx_global = si_dyn + ci
                    if idx_global < len(df):
                        thr_c = df["Throttle"].to_numpy()[idx_global]
                        brk_c = df["Brake"].to_numpy()[idx_global]
                        brk_c = brk_c * 100.0 if brk_c <= 1.0 else brk_c
                        spd_c = df["Speed"].to_numpy()[idx_global]
                        ...
                        if mode_txt:
                            ...
                            c.create_text(tx, TEL_TOP + 8, text=mode_txt, fill=YELLOW,
                                font=(FONT, 8, "bold"), anchor="w", tags="_plot_cursor")

            # Comprobar si debemos evitar el recálculo pesado
            if self._plot_last_lap == lap_num and self._plot_last_drv == drv and getattr(self, "_plot_last_w", 0) == W:
                draw_dynamic()
                return
                
            self._plot_last_lap = lap_num
            self._plot_last_drv = drv
            self._plot_last_w   = W

            # =========================================================
            # CAPA ESTÁTICA: Solo corre 1 vez por vuelta / redimensionado
            # =========================================================
            for tag in ("_plot_bg", "_plot_ers", "_plot_lines", "_plot_sec", "_plot_static_overlay", "_plot_labels", "_plot_icons"):
                c.delete(tag)

            si = int(np.searchsorted(t_vals, lap_abs_start, side='left'))
            # Extraemos data de TODA la vuelta predecida para pintar la capa estática
            ei = int(np.searchsorted(t_vals, lap_abs_start + t_span, side='right'))
            ei = min(ei + 5, len(t_vals)) # margen de seguridad
            n  = ei - si
            if n < 3: return
            ...
            if ERS_H > 0:
                ...
                soc_start = float(ers_cached[1]) if ers_cached is not None else ERS_CAPACITY_KJ * 0.90
                ...
                if len(dt_arr2) >= 2:
                    ...
                    for k in range(len(dt_arr2)):
                        ...
                        if th >= 95 and bk == 0 and sp >= ERS_DEPLOY_SPD_MIN:
                            ...
                            if sp_pct <= ERS_CLIP_SOC_THRESH: d_kw = 0.0
                            mgu_h = 40.0 * clamp((rp - 6000.) / (MAX_RPM - 6000.), 0, 1)
                            soc_p = float(np.clip(soc_p + (mgu_h - d_kw) * dt2, 0.0, ERS_CAPACITY_KJ))
                        elif bk > 20:
                ...
                if soc_pcts:
                    ...
                    for k, pct2 in enumerate(soc_pcts):
                        poly += [int(xs[min(k + 1, len(xs)-1)]), SOC_Y_BOT - int((pct2 / 100.) * ERS_H)]
                    poly += [int(xs[-1]) if len(xs) > 0 else W, SOC_Y_BOT] # Cerramos hasta el final
                    if len(poly) >= 6:
                        c.create_polygon(poly, fill="#003366", outline="", tags="_plot_ers")
                    line = []
                    for k, pct2 in enumerate(soc_pcts):
                        line += [int(xs[min(k + 1, len(xs)-1)]), SOC_Y_BOT - int((pct2 / 100.) * ERS_H)]
                    if len(line) >= 4:
                        c.create_line(line, fill=ACCENT2, width=1, smooth=True, tags="_plot_ers")

                clip_y = SOC_Y_BOT - int((ERS_CLIP_SOC_THRESH / 100.) * ERS_H)
                c.create_line(0, clip_y, W, clip_y, fill=ACCENT, width=1, dash=(4, 3), tags="_plot_ers")
                c.create_text(W - 2, clip_y - 1, text="CLIP", fill=ACCENT, font=(FONT, 6, "bold"), anchor="se", tags="_plot_ers")
            ...
            if len(sec_bounds) == 1: sec_bounds = [0, W // 3, 2 * W // 3, W]
            elif len(sec_bounds) == 2: sec_bounds.append(W)
            elif len(sec_bounds) == 3: sec_bounds.append(W)
            ...
            for si_n in range(3):
                ...
                if x1 > x0:
                    c.create_rectangle(x0, TEL_TOP, x1, TEL_TOP + TEL_H, fill=SEC_FILLS[si_n + 1], outline="", tags="_plot_sec")
                if si_n > 0:
                    c.create_line(x0, TEL_TOP, x0, TEL_TOP + TEL_H, fill=sec_line_cols[si_n], width=1, dash=(5, 3), tags="_plot_sec")
                lx = x0 + 3
                c.create_text(lx, TEL_TOP + TEL_H - 2, text=sec_labels[si_n], fill=sec_line_cols[si_n], font=(FONT, 7, "bold"), anchor="sw", tags="_plot_sec")
            ...
            sm_arr   = df["StraightMode"].to_numpy()[si:ei] if "StraightMode" in df.columns else None
            om_arr   = df["OvertakeMode"].to_numpy()[si:ei] if "OvertakeMode" in df.columns else None

            for k in range(n):
                thr_k = throttle[k]; brk_k = brake[k]; spd_k = speeds[k]; rpm_k = rpm_arr[k]
                om_on = bool(om_arr[k]) if om_arr is not None else False
                sm_on = (bool(sm_arr[k]) if sm_arr is not None else (_drs_is_open(float(drs_arr[k])) if drs_arr is not None else False))
                is_sc = (thr_k >= 95 and brk_k == 0 and spd_k >= ERS_DEPLOY_SPD_MIN and rpm_k >= 10_500 and dspd_arr[k] < -1.0)
                is_lc = (thr_k == 0 and brk_k == 0 and spd_k > 200 and dspd_arr[k] < -0.5)

                if om_on: mode_arr[k] = 2
                elif sm_on: mode_arr[k] = 1
                elif is_sc: mode_arr[k] = 3
                elif is_lc: mode_arr[k] = 4
            ...
            while k < n:
                ...
                if m == 0:
                    k += 1; continue
                j = k + 1
                while j < n and mode_arr[j] == m: j += 1
                x0 = int(xs[k])
                x1 = int(xs[min(j - 1, len(xs)-1)])
                if x1 < x0: x1 = x0 + 1
                c.create_rectangle(x0, 0, x1, LABEL_H, fill=MODE_BG[m], outline="", tags="_plot_static_overlay")
                run_w = x1 - x0
                if run_w >= MIN_ICON_W:
                    cx_icon = (x0 + x1) // 2
                    c.create_text(cx_icon, LABEL_H // 2, text=MODE_ICON[m], fill=MODE_FG[m], font=(FONT, 7, "bold"), anchor="center", tags="_plot_icons")
                k = j
            ...
            for k in range(0, n, step):
                ...
                spd_pts += [int(xs[k]), y]
            if len(spd_pts) >= 4:
                c.create_line(spd_pts, fill=F1_WHITE, width=2, smooth=True, tags="_plot_lines")
            ...
            for k in range(0, n, step):
                ...
                thr_pts += [int(xs[k]), y]
            if len(thr_pts) >= 4:
                c.create_line(thr_pts, fill=GREEN, width=1, smooth=True, tags="_plot_lines")
            ...
            for k in range(0, n, step):
                if brake[k] > 50:
                    x1b = int(xs[k])
                    x2b = x1b + max(2, step)
                    c.create_rectangle(x1b, BRK_ZONE_Y0, x2b, BRK_ZONE_Y1, fill=ACCENT, outline="", tags="_plot_lines")
            ...
            for k in range(0, n, step):
                ...
                if is_lc_k and not in_lc:
                    ...
                    lc_x1 = int(xs[k])
                    if lc_x1 > lc_x0:
                        c.create_rectangle(lc_x0, TEL_TOP, lc_x1, TEL_TOP + TEL_H, fill="#00001a", outline="", stipple="gray25", tags="_plot_lines")

            PROG_Y = H - 4
            c.create_rectangle(0, PROG_Y, W, H, fill="#111122", outline="", tags="_plot_static_overlay")
            
            c.create_text(W - 4, TEL_TOP + 3, text=f"LAP {lap_num}", fill=TEXT_SEC, font=(FONT, 8, "bold"), anchor="ne", tags="_plot_labels")
            for i, (txt, clr) in enumerate((("SPD", F1_WHITE), ("THR", GREEN), ("BRK", ACCENT))):
                c.create_text(4 + i * 36, PROG_Y - 2, text=f"▬{txt}", fill=clr, font=(FONT, 7, "bold"), anchor="sw", tags="_plot_labels")
            if ERS_H > 0:
                c.create_text(W - 4, PROG_Y - 2, text="▬ERS", fill=ACCENT2, font=(FONT, 7, "bold"), anchor="se", tags="_plot_labels")

            icon_x = 4
            for m_id, icon, fg_c in ((1, "SM", "#00cc44"), (2, "OM⚡", PURPLE), (3, "⚡SC", ACCENT), (4, "L&C", ACCENT2)):
                c.create_text(icon_x, LABEL_H // 2, text=icon, fill=fg_c, font=(FONT, 6, "bold"), anchor="w", tags="_plot_labels")
                icon_x += len(icon) * 6 + 8

            # Al terminar de dibujar lo estático, mandamos llamar el renderizado dinámico
            draw_dynamic()
