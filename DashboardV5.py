"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   F1 PRO DASHBOARD  v6.2  ·  Dear PyGui  ·  Industrial Cockpit Edition     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  pip install dearpygui fastf1 pandas numpy                                  ║
║  python f1_dashboard_v62.py                                                 ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  V6.2 vs V6.1:                                                               ║
║  • CRASH FIX: Timing rows usan generation counter en sus tags.              ║
║    Rebuild sólo cuando el orden de posiciones cambia (overtake).            ║
║    Cero colisiones de tags garantizado.                                      ║
║  • OVERTAKE SYSTEM: Se trackean overtakes en tiempo real. Analysis          ║
║    muestra: # rebases totales, posición ganada/perdida, velocidad máxima   ║
║    al rebasar, lap del último rebase y driver rebased.                      ║
║  • FULL-ROW CLICK: Timing tower usa add_selectable(span_columns=True)      ║
║    en primera columna → toda la fila es clickeable, sin botón ">" al       ║
║    final. Fila se ilumina al hover como una tabla real.                     ║
║  • HUD CUADRADO: Drawlist ahora es 255×255 (cuadrado). Arc cx/cy centrado  ║
║    correctamente, radio calculado para no salirse de bounds. Los textos     ║
║    RPM/mph quedan arriba/abajo del arco sin overflow.                       ║
║  • RACE STRATEGY: Panel ahora con scroll vertical, no se sale de pantalla. ║
║  • SPEED STAT: Track de velocidad máxima por piloto, se muestra en HUD.   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
BG0 = _c("050510"); BG1 = _c("09091a"); BG2 = _c("0d0d20")
BG3 = _c("121226"); BG4 = _c("181830")

RED  = _c("e10600"); BLUE  = _c("00a0e9"); GREEN = _c("00e676")
YLW  = _c("ffd600"); ORG   = _c("ff6d00"); PRP   = _c("d500f9")
CYAN = _c("00e5ff"); PINK  = _c("ff3d71")

TXT  = _c("eaeaf8"); TMED  = _c("8888aa"); TDIM  = _c("484860")
BRD  = _c("1c1c36"); BRDL  = _c("262640")
NONE = (0, 0, 0, 0)

F1_PRP = _c("d500f9"); F1_GRN = _c("00e676")
F1_YLW = _c("ffd600"); F1_WHT = _c("eaeaf8")

TEAM_COLORS: Dict[str, Tuple] = {
    "Mercedes":                       _c("00d2be"),
    "Ferrari":                        _c("dc0000"),
    "Red Bull Racing":                _c("3671c6"),
    "McLaren":                        _c("ff8700"),
    "Aston Martin":                   _c("358c75"),
    "Aston Martin Aramco Cognizant":  _c("358c75"),
    "Alpine":                         _c("0090ff"),
    "Williams":                       _c("37bedd"),
    "RB":                             _c("6692ff"),
    "Kick Sauber":                    _c("52e252"),
    "Haas F1 Team":                   _c("b6babd"),
    "Haas":                           _c("b6babd"),
    "Alfa Romeo":                     _c("c92d4b"),
    "AlphaTauri":                     _c("2b4562"),
}

TYRE_COL: Dict[str, Tuple] = {
    "S": YLW, "M": TXT, "H": _c("cccccc"),
    "I": GREEN, "W": BLUE, "U": TDIM,
}
TYRE_FULL: Dict[str, str] = {
    "S": "SOFT", "M": "MEDIUM", "H": "HARD",
    "I": "INTER", "W": "WET", "U": "UNK",
}
TYRE_SHORT = {"S": "S", "M": "M", "H": "H", "I": "I", "W": "W", "U": "?"}

TRACK_STATUS: Dict[str, Tuple[str, Tuple, Tuple]] = {
    "1": ("GREEN FLAG",   GREEN, _c("002200", 180)),
    "2": ("YELLOW FLAG",  YLW,   _c("222200", 180)),
    "3": ("SECTOR YEL",   YLW,   _c("222200", 180)),
    "4": ("SAFETY CAR",   ORG,   _c("221100", 180)),
    "5": ("RED FLAG",     RED,   _c("220000", 180)),
    "6": ("VIRTUAL SC",   PRP,   _c("110022", 180)),
    "7": ("VSC END",      GREEN, _c("002200", 180)),
}

SESSION_INFO: Dict[str, Tuple[str, Tuple]] = {
    "FP1": ("FREE PRACTICE 1",   BLUE),
    "FP2": ("FREE PRACTICE 2",   BLUE),
    "FP3": ("FREE PRACTICE 3",   BLUE),
    "Q":   ("QUALIFYING",        YLW),
    "SQ":  ("SPRINT QUALIFYING", ORG),
    "S":   ("SPRINT",            ORG),
    "R":   ("RACE",              RED),
}
...
# V6.3: Pre-allocated timing rows — NEVER deleted/created during session
MAX_DRV        = 22
MAX_STYLE_ROWS = 15   # pre-allocated driving style history rows
# Race mode columns (9). Quali uses first 7.
_RACE_COLS  = [("P",30),("+/-",28),("DRV",40),("Team",86),
               ("Gap",62),("Int",60),("Last",72),("Tyr",32),("L",24)]
_QUAL_COLS  = [("P",30),("DRV",40),("Team",86),("Best",72),
               ("Gap",72),("Tyr",32),("L",24)]
...
def _drs_open(v) -> bool:
    vi = int(safe(v, 0))
    return vi in DRS_ACTIVE or (vi > 8 and vi % 2 == 0)
...
def strip_fastf1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if pd.api.types.is_timedelta64_dtype(dtype):
            ...
            secs = ns / 1e9
            secs[ns == np.iinfo(np.int64).min] = float("nan")
            res[col] = secs
        elif pd.api.types.is_datetime64_any_dtype(dtype):
            try:
                ns = (arr.dt.view(np.int64).to_numpy().astype(float)
                      if hasattr(arr, "dt")
                      else arr.values.astype(np.int64).astype(float))
                secs = ns / 1e9
                secs[ns == np.iinfo(np.int64).min] = float("nan")
                res[col] = secs
            except:
                res[col] = arr.to_numpy(dtype=object, na_value=float("nan"))
        else:
...
def add_timesec_col(df: pd.DataFrame, col: str = "Time") -> pd.DataFrame:
    ...
    if not d.empty and col in d.columns:
        d["TimeSec_f"] = d[col].to_numpy(dtype=float)
    return d

def _simulate_ers(soc_kj, thr, brk, spd, rpm, dt):
    dep_acc = reg_acc = 0.0; clip = False
    n = len(dt)
    ...
    for k in range(n):
        ...
        if on[k]:
            if p <= ERS_CLIP_TH:
                d = 0.0; clip = True; net = (float(r_m[k]) + 110.0) * dk
            else:
...
class Snap:
    ...
    gap_num: float = 999.0; int_num: float = 999.0
    speed: float = 0.0; rpm: float = 0.0
    throttle: float = 0.0; brake: float = 0.0
    gear: int = 0; drs: float = 0.0
    straight_mode: bool = False; overtake_mode: bool = False
    tyre: str = "–"; tyre_prev: str = "–"; tyre_laps: int = 0
    dist: float = -1.0; is_out: bool = False
    ...
    sector_times: Tuple = (0.0, 0.0, 0.0)
    pits_done: int = 0; pos_change: int = 0
...
class SnapshotWorker(threading.Thread):
    ...
    def run(self):
        while not self._stop.is_set():
            try:
                ...
                log.warning("Worker exception: %s", exc, exc_info=True)
    ...
    def _compute(self, elapsed: float) -> dict:
        ...
        if (app.track_status_df is not None and not app.track_status_df.empty
            ...
            if idx >= 0:
                ts_key = str(app.track_status_df["Status"].iloc[idx])
        ...
        for drv, df in app.telemetry.items():
            ...
            sn = Snap(
                ...
                speed=g("Speed"), rpm=g("RPM"),
                throttle=g("Throttle"), brake=_norm_brk(g("Brake")),
                gear=int(g("nGear")), drs=g("DRS"),
                x=g("X", float("nan")), y=g("Y", float("nan")), is_out=is_out,
            )
            if "StraightMode" in df.columns:
                try:
                    sm = df["StraightMode"].to_numpy()[idx]
                    sn.straight_mode = bool(sm) if sm == sm else False
                except: pass
            if "OvertakeMode" in df.columns:
                try:
                    om = df["OvertakeMode"].to_numpy()[idx]
                    sn.overtake_mode = bool(om) if om == om else False
                except: pass

            # ── V6.5: Official timing lookup ─────────────────────────────
            # All position/gap/interval come from FastF1 laps — zero computation.
            ot = app._off_timing.get(drv, {})
            ot_t = ot.get("t", np.array([]))
            if len(ot_t) > 0:
                # Find index of latest COMPLETED lap at abs_t
                oi = int(np.searchsorted(ot_t, abs_t, side="right")) - 1
                oi = max(0, min(oi, len(ot_t) - 1))
                pos_f = ot["pos"][oi] if oi < len(ot.get("pos",[])) else 999.0
                sn.pos = int(safe(pos_f, 999))
                lap_f  = ot["lap"][oi] if oi < len(ot.get("lap",[])) else 0.0
                sn.lap = int(safe(lap_f, 0))
                # Gap/Interval — official seconds from FastF1
                gap_s  = ot["gap"][oi] if oi < len(ot.get("gap",[])) else float("nan")
                ivl_s  = ot["ivl"][oi] if oi < len(ot.get("ivl",[])) else float("nan")
                sn.gap_num = float(safe(gap_s, 999.0))
                sn.int_num = float(safe(ivl_s, 999.0))
                # Format for display
                if sn.is_out:
                    sn.gap = sn.interval = "OUT"
                elif math.isnan(gap_s) or gap_s <= 0:
                    sn.gap = "LEAD"; sn.gap_num = 0.0
                else:
                    sn.gap = f"+{gap_s:.1f}s" if gap_s < 120 else "+1 Lap"
                if sn.is_out:
                    sn.interval = "OUT"
                elif math.isnan(ivl_s) or ivl_s <= 0:
                    sn.interval = "LEAD"; sn.int_num = 0.0
                else:
                    sn.interval = f"+{ivl_s:.1f}s" if ivl_s < 120 else "+1 Lap"
                # Last lap time
                lt_s = ot["lt"][oi] if oi < len(ot.get("lt",[])) else float("nan")
                if not math.isnan(lt_s) and lt_s > 0:
                    sn.last_lap_str = fmt_lap(lt_s)
                # Best lap
                lt_all = ot.get("lt", np.array([]))[:oi+1]
                valid_lt = lt_all[~np.isnan(lt_all)]
                if len(valid_lt):
                    sn.best_lap_str = fmt_lap(float(np.min(valid_lt)))

            # ── Remaining lap data (tyre, pit, sectors) ──────────────────
            try:
                ...
                if not dl.empty and "TimeSec_f" in dl.columns and len(lta) > 0:
                    ...
                    if comp is not None:
                        cr = str(comp)
                        if cr not in ("nan", "None", ""):
                            sn.tyre = f"({cr[0].upper()})"
                    if cli > 0:
                    ...
                    if "PitInTime" in dl.columns and "PitOutTime" in dl.columns:
                        ...
                        for li in range(len(pit_in)):
                            ...
                            pi_abs = app.session_start + pi_t if not math.isnan(pi_t) else float("nan")
                            po_abs = app.session_start + po_t if not math.isnan(po_t) else float("nan")
                            if not math.isnan(pi_abs):
                                w_end = po_abs + 8 if not math.isnan(po_abs) else pi_abs + 65
                                if pi_abs - 2 <= abs_t <= w_end:
                        ...
                        if not sn.in_pit and "PitOutTime" in dl.columns:
                            ...
                            if cli < len(pit_out2):
                                ...
                                if not math.isnan(pov) and abs_t >= app.session_start + pov - 5:
                        ...
                        sn.pits_done = int(np.sum(
                            np.array([app.session_start + float(p) for p in pots]) <= abs_t))
                ...
                log.warning("Lap data drv=%s: %s", drv, exc)
            ...
            if is_2026(app.session_year) and not is_out:
                try:
                    ...
                    ra = (df["RPM"].to_numpy().astype(float)
                          if "RPM" in df.columns else np.zeros(len(times)))
                    if prev_ers is None or abs_t < prev_ers[0] - 0.1:
                        ts = times[:i_now+1]; dts = np.diff(ts)
                        soc, dep, reg, clip = _simulate_ers(
                            ERS_CAP_KJ * 0.90,
                            ta[:i_now+1], ba[:i_now+1], sa[:i_now+1], ra[:i_now+1], dts)
                    else:
                        pr_t, soc, dep, reg, clip = prev_ers
                        i_pr = max(0, min(int(np.searchsorted(times, pr_t)), len(times)-1))
                        ts2 = times[i_pr:i_now+1]; dts2 = np.diff(ts2)
                        if len(dts2) > 0:
                            s_soc, s_dep, s_reg, clip = _simulate_ers(
                                soc, ta[i_pr:i_now+1], ba[i_pr:i_now+1],
                                sa[i_pr:i_now+1], ra[i_pr:i_now+1], dts2)
                            soc = s_soc; dep += s_dep; reg += s_reg
                    sn.ers_soc = clamp((soc / ERS_CAP_KJ) * 100, 0, 100)
                    sn.is_clipping = clip
                    app._ers_state[drv] = (abs_t, soc, dep, reg, clip)
                except Exception as exc:
                    log.debug("ERS %s: %s", drv, exc)

            snaps.append(sn)

        # V6.5: Sort by official Position from FastF1. Stable — only changes once per lap.
        # Drivers with no valid position (pos=999) go to back.
        # OUT drivers always last.
        def _sort_key(s: Snap) -> int:
            if s.is_out: return 9999
            return s.pos if 0 < s.pos < 900 else 998

        snaps.sort(key=_sort_key)

        # pos_change: positive = gained positions since last frame
...
class F1App:
    ...
    def __init__(self):
        ...
        self._wx_times           = np.array([])
        self._tel_times:         Dict[str, np.ndarray] = {}
        self._lap_times:         Dict[str, np.ndarray] = {}
        self._laps_by_drv:       Dict[str, pd.DataFrame] = {}
        self._off_timing:        Dict[str, dict] = {}          # V6.5: official timing data
        self._sector_bests:      Dict[str, List] = {}
        self._overall_sec_best:  List = [None, None, None]
        self._ers_state:         Dict[str, tuple] = {}
        self._timing_mode:       str   = "race"

        # Track map
        self._track_x = None; self._track_y = None
        self._map_dots: set = set()
        self._map_zoom: float = 1.0
        self._map_pan_x: float = 0.0; self._map_pan_y: float = 0.0
        ...
        self._session_q:      queue.Queue = queue.Queue()  # session/calendar results → main thread (unlimited, never blocks bg thread)
        ...
        # Analysis
        self._ana_last_lap      = object()
        self._ana_last_drv      = ""
        self._ana_last_style_n  = -1
        self._prev_positions:   Dict[str, int] = {}
        self._pit_entry_time:   Dict[str, float] = {}

        # V6.3: Timing — pre-allocated stable rows, zero crash risk
        # _timing_slots[i] = driver id currently in row slot i ("" = empty/hidden)
        self._timing_slots:     List[str] = [""] * MAX_DRV
        self._timing_mode:      str = "race"   # "race" | "quali"
        self._track_lap_len:    float = 5500.0  # updated per session from telemetry

        # V6.2: Overtake tracking
        self._overtake_count:   Dict[str, int] = {}    # total overtakes per driver
        self._pos_lost_count:   Dict[str, int] = {}    # positions lost per driver
        self._overtake_events:  List[Tuple] = []       # (elapsed, drv_overtaker, drv_overtaken, lap)
        self._last_overtake_str: Dict[str, str] = {}   # last overtake description per driver
        self._session_max_speed: Dict[str, float] = {} # max speed ever seen per driver

        # Dirty flags
        self._strategy_last_hash = ""
        self._ers_last_pct       = -1.0
        ...
        # HUD geometry
        self._hud_W = 0; self._hud_H = 0
        self._hud_cx = 0; self._hud_cy = 0
        self._hud_r_rpm = 0; self._hud_r_inner = 0
        self._hud_gear_str = "N"
        self._hud_spd_val = 0; self._hud_rpm_val = 0
        self._hud_font: Optional[int] = None
    ...
    def run(self):
        ...
        dpg.create_viewport(
            title="F1 Pro Dashboard  v6.2  ·  Dear PyGui",
        ...
        if self._hud_font is not None:
            try:
                dpg.bind_item_font("cw_hud", self._hud_font)
                log.info("HUD font bound")
            except Exception as e:
                log.warning("HUD font bind: %s", e)
        ...
        with dpg.font_registry():
            for path in _FONT_CANDIDATES:
                if os.path.exists(path):
                    try:
                        self._hud_font = dpg.add_font(path, 16)
                        log.info("Font: %s", os.path.basename(path))
                        return
    ...
    def _apply_theme(self):
        with dpg.theme() as gt:
            with dpg.theme_component(dpg.mvAll):
                ...
                dpg.add_theme_color(dpg.mvThemeCol_HeaderHovered,     _c("e10600", 80))
                dpg.add_theme_color(dpg.mvThemeCol_HeaderActive,      _c("e10600", 200))
                ...
                # Selectable highlight = very subtle team-neutral colour
        ...
        self._th_spd = _sth(TXT,   2.0)
        self._th_thr = _sth(GREEN, 1.5)
        self._th_brk = _sth(RED,   1.5)
        self._th_ers = _sth(BLUE,  1.0)
        self._th_cur = _sth(YLW,   2.0)
    ...
    def _ui_header(self):
        with dpg.child_window(height=HDR_H, no_scrollbar=True, border=False):
            with dpg.group(horizontal=True):
                dpg.add_text(" F1", color=RED)
                dpg.add_text("PRO", color=TXT)
                dpg.add_text("DASHBOARD", color=TMED)
                dpg.add_text("v6.2", color=TDIM)
                dpg.add_spacer(width=18)
                dpg.add_text("Year", color=TDIM)
                dpg.add_combo([str(y) for y in range(2018, 2027)],
                              default_value="2024", width=72, tag="cb_year",
                              callback=lambda s, a: self._load_calendar(int(a)))
                dpg.add_spacer(width=8)
                dpg.add_text("Race", color=TDIM)
                dpg.add_combo([], width=280, tag="cb_race")
                dpg.add_spacer(width=8)
                dpg.add_text("Sess", color=TDIM)
                dpg.add_combo(["FP1","FP2","FP3","Q","SQ","S","R"],
                              default_value="R", width=60, tag="cb_sess",
    ...
    def _ui_controls(self):
        with dpg.child_window(height=CTL_H, no_scrollbar=True, border=False):
            with dpg.group(horizontal=True):
                ...
                dpg.add_spacer(width=8)
                dpg.add_text("Speed:", color=TDIM)
                dpg.add_combo(["x1","x4","x16","x64"], default_value="x1",
                              tag="cb_speed", width=62,
                              callback=lambda s,a: self._set_speed(
                                  {"x1":1,"x4":4,"x16":16,"x64":64}.get(a,1)))
                dpg.add_spacer(width=12)
                ...
                with dpg.group():
                    ...
                    dpg.add_slider_float(tag="sld_time", default_value=0,
                                        ...
                                        width=880, callback=self._on_slider,
                                        format="", no_input=True)
                dpg.add_spacer(width=10)
    ...
    def _ui_race_admin(self):
        ...
        with dpg.child_window(height=260, border=False, no_scrollbar=False):
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("v", color=ORG)
                dpg.add_text("0 kW", tag="txt_deploy", color=TDIM)
                dpg.add_spacer(width=16)
                dpg.add_text("^", color=GREEN)
                dpg.add_text("0 kW", tag="txt_regen", color=TDIM)
    ...
    def _ui_map(self):
        with dpg.child_window(width=MAP_W, height=396, border=True):
            with dpg.group(horizontal=True):
                dpg.add_text("TRACK MAP", color=RED)
                dpg.add_spacer(width=8)
                dpg.add_text("—", tag="txt_track_name", color=TMED)
                dpg.add_spacer(width=10)
    ...
    def _ui_hud(self):
        # V6.2: 310px wide, 396px tall. Drawlist is 255×255 (square gauge)
        with dpg.child_window(width=HUD_W, height=396, border=True, tag="cw_hud"):
            ...
            dpg.add_separator()
            with dpg.group(horizontal=True):
                dpg.add_text("AHD:", color=TDIM)
                dpg.add_text("—", tag="txt_hud_ahead", color=CYAN)
            with dpg.group(horizontal=True):
                dpg.add_text("BHD:", color=TDIM)
                dpg.add_text("—", tag="txt_hud_behind", color=ORG)
            ...
            dpg.add_separator()
            dpg.add_text("—", tag="txt_hud_drs", color=TDIM)
            dpg.add_text("",  tag="txt_hud_om",  color=TDIM)
            dpg.add_separator()
            # V6.2: 255×210 — square-ish, fits the arc perfectly
            DL_W = HUD_W - 18
    ...
    def _hud_init_arcs(self, W, H):
        self._hud_W = W; self._hud_H = H
        # V6.2: center slightly above midpoint so text below has room
        self._hud_cx = W * 0.48
        self._hud_cy = H * 0.48
        # V6.2: radius bounded so the arc never exits the drawlist
        margin = 18
        self._hud_r_rpm   = min(W * 0.5 - margin, H * 0.5 - margin)
        self._hud_r_inner = self._hud_r_rpm - 17
        self._hud_draw_static_bg()

    def _hud_draw_static_bg(self):
        cx, cy   = self._hud_cx, self._hud_cy
        r_rpm    = self._hud_r_rpm
        r_inner  = self._hud_r_inner
        ...
        for pct in (0, 25, 50, 75, 100):
            deg = pct / 100 * 240
            rad = math.radians(180.0 - deg)
            x1 = cx + (r_rpm - 3) * math.cos(rad)
            y1 = cy - (r_rpm - 3) * math.sin(rad)
            x2 = cx + (r_rpm + 9) * math.cos(rad)
            y2 = cy - (r_rpm + 9) * math.sin(rad)
            dpg.draw_line((x1,y1),(x2,y2), color=_c("2a2a4a"), thickness=1, parent="dl_hud")

        cx_i, cy_i = int(cx), int(cy)

        # Dynamic draw items — created ONCE, reconfigured every frame (no delete/create)
    ...
    def _hud_redraw_dynamic(self, rpm_pct, thr_pct, brk_on, redline):
        ...
        if rpm_deg > 1.0:
            segs = max(4, int(rpm_deg / 4))
            pts  = self._arc_pts(cx, cy, r_rpm, 0, rpm_deg, segs)
            if pts:
                mid = len(pts) // 2
                ...
                dpg.configure_item("arc_rpm_lo", points=[(cx,cy)])
                dpg.configure_item("arc_rpm_hi", points=[(cx,cy)])
        else:
            dpg.configure_item("arc_rpm_lo", points=[(cx,cy)])
            dpg.configure_item("arc_rpm_hi", points=[(cx,cy)])
        ...
        if redline:
            ...
            dpg.configure_item("arc_rpm_red", points=[(cx,cy)])
        ...
        if thr_deg > 1.0:
            ...
            dpg.configure_item("arc_thr", points=[(cx,cy)])

        if brk_on:
            pts = self._arc_pts(cx, cy, r_inner, 200, 240, 20)
            dpg.configure_item("arc_brk", points=pts if pts else [(cx,cy)], color=RED, thickness=10)
        else:
            dpg.configure_item("arc_brk", points=[(cx,cy)])

        gear  = self._hud_gear_str
        spd   = self._hud_spd_val
        rpm   = self._hud_rpm_val
        H     = self._hud_H

        gear_col = CYAN if gear not in ("N","0","") else TDIM
        if redline: gear_col = RED
        g_size = 42
        g_x = int(cx - (g_size * 0.27 if len(gear)==1 else g_size * 0.52))
        g_y = int(cy - g_size // 2 + 2)
        dpg.configure_item("draw_gear", pos=(g_x, g_y), text=gear, color=gear_col)

        spd_str = str(spd); s_cw = 15
        s_x = int(cx - len(spd_str) * s_cw // 2)
        s_y = int(cy + r_inner * 0.38)
        dpg.configure_item("draw_spd", pos=(s_x, s_y), text=spd_str)
        dpg.configure_item("draw_kph", pos=(s_x + len(spd_str)*s_cw + 2, s_y + 4))

        mph_str = f"{int(spd * 0.621371)} mph"
        m_x = int(cx - len(mph_str) * 7 // 2)
        dpg.configure_item("draw_mph", pos=(m_x, s_y + 26), text=mph_str)
        dpg.configure_item("draw_rpm", pos=(4, H - 14), text=f"{rpm:,} rpm")

    # ─────────────────────────────────────────────────────────────────
    def _ui_plot(self):
        with dpg.plot(tag="plot_tel", height=138, width=-1,
        ...
        for tag, th in (("s_spd",self._th_spd),("s_thr",self._th_thr),
                        ("s_brk",self._th_brk),("s_ers",self._th_ers),
                        ("s_cur",self._th_cur)):
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.child_window(width=PACE_W, height=PH, border=True):
                    ...
                    with dpg.table(tag="tbl_pace_metrics", header_row=False,
                        ...
                        for _ in range(4):
                            dpg.add_table_column(width_fixed=True, init_width_or_weight=CW)
                        with dpg.table_row():
                            dpg.add_text("Deg/lap",  color=TDIM)
                            dpg.add_text("Gap P1",   color=TDIM)
                            dpg.add_text("Stint avg",color=TDIM)
                            dpg.add_text("Undercut", color=TDIM)
                        with dpg.table_row():
                            dpg.add_text("—", tag="pm_deg",   color=TXT)
                            dpg.add_text("—", tag="pm_gapp1", color=TXT)
                            dpg.add_text("—", tag="pm_savg",  color=TXT)
                            dpg.add_text("—", tag="pm_ucut",  color=TXT)
                        with dpg.table_row():
                            dpg.add_text("Overtakes", color=TDIM)
                            dpg.add_text("Pos lost",  color=TDIM)
                            dpg.add_text("Last OVT",  color=TDIM)
                            dpg.add_text("MaxSpd",    color=TDIM)
                        with dpg.table_row():
                            dpg.add_text("—", tag="pm_ovt",     color=GREEN)
                            dpg.add_text("—", tag="pm_poslost", color=RED)
                            dpg.add_text("—", tag="pm_lastovt", color=YLW)
                            dpg.add_text("—", tag="pm_maxspd",  color=TXT)
                        with dpg.table_row():
                            dpg.add_text("Pit wndw", color=TDIM)
                            dpg.add_text("Laps left",color=TDIM)
                            dpg.add_text("Vs best",  color=TDIM)
                            dpg.add_text("Track",    color=TDIM)
                        with dpg.table_row():
                            dpg.add_text("—", tag="pm_fc_win", color=TXT)
                            dpg.add_text("—", tag="pm_fc_gap", color=TXT)
                            dpg.add_text("—", tag="pm_fc_dlt", color=TXT)
                            dpg.add_text("—", tag="pm_cond",   color=TXT)
                        with dpg.table_row():
                            dpg.add_text("Min spd",  color=TDIM)
                            dpg.add_text("Avg spd",  color=TDIM)
                            dpg.add_text("DRS use",  color=TDIM)
                            dpg.add_text("Lap type", color=TDIM)
                        with dpg.table_row():
                            dpg.add_text("—", tag="pm_minspd",  color=TXT)
                            dpg.add_text("—", tag="pm_avgspd",  color=TXT)
                            dpg.add_text("—", tag="pm_drslaps", color=TXT)
                            dpg.add_text("—", tag="pm_laptype", color=TXT)
    ...
    # ═════════════════════════════════════════════════════════════════
    def _frame(self):
        # ── Session/calendar load results (must run on main thread — DPG not thread-safe) ──
        try:
            ...
            try:
                if "_calendar" in sess_result:
                    ...
                    if races:
                        dpg.configure_item("cb_race", items=races,
                                           default_value=races[0])
                    dpg.set_value("txt_status", "Calendar loaded.")
                elif sess_result.get("ok"):
                    self._on_session_loaded(
                        sess_result["race"], sess_result["year"], sess_result["sess_type"])
                elif sess_result.get("err"):
                    dpg.set_value("txt_status", f"Error: {sess_result['err']}")
                    self._loading = False
            except Exception as exc:
                log.warning("session_q dispatch: %s", exc, exc_info=True)
                self._loading = False
        except queue.Empty:
            pass
        try:
            ...
            try:
                ...
                log.warning("_apply_result: %s", exc)
        except queue.Empty:
            pass
        ...
        if self.is_playing and self._session_loaded:
            now = time.monotonic()
            dt  = (now - self._last_tick) * int(self.playback_speed or 1)
            self._last_tick = now
            ...
            # Paused: refresh timing table ~once per second so colors/selection stay current
    ...
    def _apply_result(self, res: dict):
        ...
        try:
            self._detect_overtakes(snaps, elapsed)
        except Exception as exc:
            log.warning("detect_overtakes: %s", exc, exc_info=True)

        if "new_positions" in res:
            self._prev_positions.update(res["new_positions"])
        ...
        try:
            ...
            if res["weather_txt"]:
                dpg.set_value("txt_weather", res["weather_txt"])
        except Exception as exc:
            log.warning("apply header: %s", exc, exc_info=True)
        ...
        try:
            ...
            log.warning("max speed: %s", exc)

        try:
            self._update_timing(snaps, abs_t)
        except Exception as exc:
            log.warning("update_timing: %s", exc, exc_info=True)

        try:
            self._update_map_dots(snaps)
        except Exception as exc:
            log.warning("update_map: %s", exc)
        ...
        try:
            self._update_hud(snap, snaps)
        except Exception as exc:
            log.warning("update_hud: %s", exc, exc_info=True)

        try:
            self._update_race_admin(snap)
        except Exception as exc:
            log.warning("update_race_admin: %s", exc, exc_info=True)

        try:
            self._update_plot(drv, abs_t, snap)
        except Exception as exc:
            log.warning("update_plot: %s", exc, exc_info=True)

        try:
            self._update_analysis(snap, drv, abs_t)
        except Exception as exc:
            log.warning("update_analysis: %s", exc, exc_info=True)

    # ─────────────────────────────────────────────────────────────────
    #  V6.2: OVERTAKE DETECTION
    # ─────────────────────────────────────────────────────────────────
    def _detect_overtakes(self, snaps: List[Snap], elapsed: float):
        """Detect real on-track overtakes. Only counts when gap was small (<= 5s).
        Lap-crossing position jumps don't count because interval would be large."""
        if not self._prev_positions: return
        for i, s in enumerate(snaps):
            if s.is_out or s.in_pit or s.pit_out_lap: continue
            prev = self._prev_positions.get(s.drv, i + 1)
            curr = i + 1
            # Position improved
            if prev > curr:
                # Only count as overtake if interval to car behind was close
                # (large gap = lap-crossing artefact, not a real overtake)
                interval_ok = s.int_num <= 5.0 or (i + 1 < len(snaps) and snaps[i + 1].int_num <= 5.0)
                if interval_ok:
                    self._overtake_count[s.drv] = self._overtake_count.get(s.drv, 0) + 1
                    if i + 1 < len(snaps):
                        overtaken = snaps[i + 1]
                        self._last_overtake_str[s.drv] = f"P{curr} vs {overtaken.abbr} L{s.lap}"
                    self._overtake_events.append((elapsed, s.drv, s.lap))
            # Position lost (not from pit or lap end)
            elif curr > prev:
                interval_ok = s.gap_num <= 5.0 or s.int_num <= 5.0
                if interval_ok:
                    self._pos_lost_count[s.drv] = self._pos_lost_count.get(s.drv, 0) + 1

    # ─────────────────────────────────────────────────────────────────
    #  V6.2: TIMING TABLE — generation counter, span_columns selectable
    # ─────────────────────────────────────────────────────────────────
    def _on_timing_row(self, sender, app_data, user_data):
        """V6.9: Extract driver from sender tag index → avoids configure_item user_data crash."""
        drv = None
        try:
            # sender is "tc_{i}_0" — extract row index, look up snap
            row_i = int(sender.split("_")[1])
            if 0 <= row_i < len(self._last_snaps):
                drv = self._last_snaps[row_i].drv
        except Exception:
            pass
        if not drv and user_data:
            drv = user_data  # fallback to stored user_data
        if drv:
            self._select_driver(drv)
        try: dpg.set_value(sender, False)
        except: pass

    def _update_timing(self, snaps: List[Snap], abs_t: float):
        """V6.4: Update pre-allocated rows in-place. ZERO DPG item creation/deletion."""
        for i in range(MAX_DRV):
            if i >= len(snaps):
                try: dpg.configure_item(f"trow_{i}", show=False)
                except: pass
                continue

            try:
                dpg.configure_item(f"trow_{i}", show=True)
            except: pass

            try:
                s        = snaps[i]
                col      = TEAM_COLORS.get(s.team, TXT)
                is_sel   = s.drv == self.selected_driver
                sel_col  = (255, 255, 255, 255) if is_sel else col
                tyre_raw = s.tyre.strip("()")

                # Tyre / pit display
                if s.in_pit:
                    self._pit_entry_time[s.drv] = abs_t
                    tyre_disp, tyre_col = "PITS", YLW
                elif s.drv in self._pit_entry_time:
                    if abs_t - self._pit_entry_time[s.drv] < 22.0:
                        tyre_disp, tyre_col = "PITS", YLW
                    else:
                ...
                pos_str = "OUT" if s.is_out else str(i + 1)

                gap_disp = "OUT" if s.is_out else ("LEAD" if i == 0 else s.gap)
                int_disp = "OUT" if s.is_out else ("LEAD" if i == 0 else s.interval)
                gap_col  = GREEN if i == 0 else TMED
                int_col  = GREEN if i == 0 else TMED

                if self._timing_mode == "race":
                    vals   = [pos_str, chg_str, s.abbr, s.team[:10],
                              gap_disp, int_disp, s.last_lap_str, tyre_disp, str(s.lap)]
                    v_cols = [col, chg_col, sel_col, col, gap_col, int_col, col, tyre_col, TDIM]
                else:
                    best  = s.best_lap_str; gap_q = "-"; gc = TMED
                    if self.session_best_s:
                        dl = self._laps_by_drv.get(s.drv, pd.DataFrame())
                        if not dl.empty and "LapTime" in dl.columns:
                            lta = dl["LapTime"].to_numpy(dtype=float)
                            vld = lta[~np.isnan(lta)]
                            if len(vld):
                                d = float(np.min(vld)) - self.session_best_s
                                gap_q = "POLE" if abs(d) < 0.01 else f"+{d:.3f}"
                                gc = F1_PRP if gap_q == "POLE" else TMED
                    vals   = [pos_str, s.abbr, s.team[:10], best, gap_q, tyre_disp, str(s.lap)]
                    v_cols = [col, sel_col, col, col, gc, tyre_col, TDIM]

                # col 0: selectable — update label only, driver derived from row index in callback
                dpg.configure_item(f"tc_{i}_0", label=str(vals[0]))

                # col 1..n: plain text items
                for ci in range(1, len(vals)):
                    dpg.set_value(f"tc_{i}_{ci}", str(vals[ci]))
                    dpg.configure_item(f"tc_{i}_{ci}", color=v_cols[ci])

                # Clear extra columns (race has 9, quali has 7)
                for ci in range(len(vals), len(_RACE_COLS)):
                    dpg.set_value(f"tc_{i}_{ci}", "")

            except Exception as exc:
                log.debug("timing row %d: %s", i, exc)
    ...
    def _update_map_dots(self, snaps: List[Snap]):
        ...
        for s in snaps:
            if s.is_out or math.isnan(s.x): continue
            sx, sy = self._w2m(s.x, s.y, W, H)
            col = TEAM_COLORS.get(s.team, TXT)
            sel = s.drv == self.selected_driver
            r   = 8.0 if sel else 4.5
            otl = (255,255,255,220) if sel else col
            ...
            if s.drv not in self._map_dots:
                ...
                try:
                    dpg.configure_item(oval, center=(sx,sy), radius=r, color=otl, fill=col)
    ...
    def _w2m(self, wx, wy, W, H):
        ...
        cx = (mx_x + mn_x) / 2 + self._map_pan_x
        cy = (mx_y + mn_y) / 2 + self._map_pan_y
    ...
    def _map_center_on_driver(self):
        ...
        if sn and not math.isnan(sn.x):
            mn_x, mx_x = self._track_x.min(), self._track_x.max()
            mn_y, mx_y = self._track_y.min(), self._track_y.max()
            self._map_pan_x = sn.x - (mx_x + mn_x) / 2
            self._map_pan_y = sn.y - (mx_y + mn_y) / 2
            self._draw_track()
    ...
    def _update_hud(self, snap: Snap, snaps: List[Snap]):
        ...
        self._hud_spd_val  = int(snap.speed)
        self._hud_rpm_val  = int(snap.rpm)
        self._hud_redraw_dynamic(
            clamp(snap.rpm / MAX_RPM, 0, 1),
            clamp(snap.throttle / 100.0, 0, 1),
            snap.brake > 0,
            snap.rpm > MAX_RPM * 0.88,
        )

        use2026 = is_2026(self.session_year)
        if use2026:
            if snap.overtake_mode:
                dpg.set_value("txt_hud_drs", "STRAIGHT MODE ON"); dpg.configure_item("txt_hud_drs", color=GREEN)
                dpg.set_value("txt_hud_om", "OVERTAKE MODE ON");  dpg.configure_item("txt_hud_om",  color=PRP)
            elif snap.straight_mode or _drs_open(snap.drs):
                dpg.set_value("txt_hud_drs", "STRAIGHT MODE ON"); dpg.configure_item("txt_hud_drs", color=GREEN)
                dpg.set_value("txt_hud_om", "OVERTAKE MODE OFF"); dpg.configure_item("txt_hud_om",  color=TDIM)
            else:
                dpg.set_value("txt_hud_drs", "STRAIGHT MODE OFF"); dpg.configure_item("txt_hud_drs", color=TDIM)
                dpg.set_value("txt_hud_om", "")
        else:
            on = _drs_open(snap.drs)
            dpg.set_value("txt_hud_drs", "DRS OPEN" if on else "DRS CLOSED")
            dpg.configure_item("txt_hud_drs", color=GREEN if on else TDIM)
            dpg.set_value("txt_hud_om", "")
        ...
        if idx >= 0:
            ...
            dpg.set_value("txt_hud_ahead",  f"{ah.abbr}  +{snap.int_num:.1f}s" if ah and not ah.is_out else "LEADER")
            dpg.set_value("txt_hud_behind", f"{bh.abbr}  +{bh.int_num:.1f}s"  if bh and not bh.is_out else "")
    ...
    def _update_battle_style(self, snap: Snap, snaps: List[Snap], idx: int):
        ...
        elif use2026 and snap.is_clipping:
            style, sc = f"SUPERCLIPPING  {snap.ers_soc:.0f}%", PRP
        elif use2026 and snap.throttle >= 95 and snap.brake == 0 and snap.speed > ERS_SPD_MIN and snap.ers_soc < 20:
        ...
        dpg.set_value("txt_hud_style", f"STYLE: {style}")
        dpg.configure_item("txt_hud_style", color=sc)

        battle, bc = "CLEAR", TDIM
        if idx < len(snaps)-1 and snaps[idx+1].int_num <= 3.0:
            battle, bc = f"DEFEND ← {snaps[idx+1].abbr}", ORG
        ...
        dpg.set_value("txt_hud_battle", f"TRACK: {battle}")
        dpg.configure_item("txt_hud_battle", color=bc)
    ...
    def _update_race_admin(self, snap: Snap):
        try:
            if snap.in_pit and snap.speed <= 90:
                ...
                dpg.set_value("txt_pit", "OUT LAP"); dpg.configure_item("txt_pit", color=BLUE)
            elif snap.lap_type == "IN LAP":
                dpg.set_value("txt_pit", "IN LAP"); dpg.configure_item("txt_pit", color=ORG)
            else:
                dpg.set_value("txt_pit", "ON TRACK"); dpg.configure_item("txt_pit", color=GREEN)

            dpg.set_value("txt_pits_done", str(snap.pits_done))
            t_raw  = snap.tyre.strip("()")
            t_full = TYRE_FULL.get(t_raw.upper(), t_raw or "—")
            dpg.set_value("txt_tyre", t_full)
            dpg.configure_item("txt_tyre", color=TYRE_COL.get(t_raw.upper(), TDIM))
            ...
            if new_hash != self._strategy_last_hash:
                ...
                except Exception as exc: log.warning("strategy timeline: %s", exc)
        except Exception as exc:
            log.warning("update_race_admin: %s", exc, exc_info=True)
            return
        ...
        try:
            ...
            if abs(pct - self._ers_last_pct) > 0.5:
                ...
                try: self._draw_ers_bar(pct, dep_kw, reg_kw)
                except Exception as exc: log.warning("ers bar: %s", exc)

            if snap.is_clipping:
                dpg.set_value("txt_ers_status", "SUPERCLIPPING"); dpg.configure_item("txt_ers_status", color=PRP)
            elif reg_kw > 5:
            ...
            log.warning("update_race_admin ERS: %s", exc)

    def _draw_ers_bar(self, pct: float, dep_kw: float, reg_kw: float):
        try:
            BW = LEFT_W - 28
            ...
            if fw > 0:
                ...
                dpg.draw_rectangle((0,0),(fw,3), fill=_c("ccffdd") if pct > 60 else _c("ffeeaa"), color=NONE, parent="dl_ers_bar")
            ...
            log.debug("ers_bar: %s", exc)

    def _draw_strategy_timeline(self, drv: str, snap: Snap):
        """V6.9: Update pre-allocated strategy rect tags. ZERO item creation/deletion."""
        try:
            BW = LEFT_W - 28
            ...
            cur  = float(snap.lap)
            total = float(self.total_laps) if self.total_laps > 0 else 1.0
            cx = cur / total * BW
            ...
            if not dl.empty and "Compound" in dl.columns:
                ...
                lapn  = dl["LapNumber"].to_numpy(dtype=float)
                ...
                if prev_c is not None:
                    stints.append((stint_s, int(safe(lapn[-1], stint_s)), str(prev_c)))

            # Redraw: clear and rebuild (drawlist items, not table items — this is safe)
            ...
            for s_lap, e_lap, comp_str in stints:
                ...
                if x1 - x0 > 14:
                    dpg.draw_text((x0+3, 8), t_raw, color=_c("000000"), size=12, parent="dl_strategy")
            dpg.draw_line((cx,0),(cx,28), color=TXT, thickness=2, parent="dl_strategy")
        except Exception as exc:
            log.debug("strategy_timeline: %s", exc)
    ...
    def _update_plot(self, drv: str, abs_t: float, snap: Snap):
        try:
            ...
            if idx_c > 0 and lt_col is not None:
                ...
                lap_start = self.session_start + pe if (not math.isnan(pe) and not math.isnan(pl)) else float(tval[0])
                t_span    = pl if (not math.isnan(pl) and pl > 0) else 90.0
            else:
                lap_start = float(tval[0]); t_span = 90.0
            ...
            if lap_changed:
                ...
                try:
                    ...
                    step = max(1, n // 600)
                    dpg.set_value("s_spd", [t[::step], spd[::step].tolist()])
                    dpg.set_value("s_thr", [t[::step], thr[::step].tolist()])
                    dpg.set_value("s_brk", [t[::step], np.clip(brk, 0, 100)[::step].tolist()])
                    dpg.set_value("s_ers", [[], []])
                    dpg.set_axis_limits("ax_x", 0, t_span)
                except Exception as exc:
                    log.warning("plot data: %s", exc, exc_info=True)

                try:
                    st = snap.sector_times
                    if st and len(st) == 3 and all(s > 0 for s in st):
                        s1 = float(st[0]); s2 = s1 + float(st[1]); s3 = s2 + float(st[2])
                        YMAX = 430.0
                        dpg.set_value("s_sec1", [[0,s1,s1,0],[YMAX,YMAX,0,0]])
                        dpg.set_value("s_sec2", [[s1,s2,s2,s1],[YMAX,YMAX,0,0]])
                        dpg.set_value("s_sec3", [[s2,s3,s3,s2],[YMAX,YMAX,0,0]])
                    else:
                        for stag in ("s_sec1","s_sec2","s_sec3"): dpg.set_value(stag, [[],[]])
                except Exception as exc:
                    log.warning("plot sectors: %s", exc)
                    for stag in ("s_sec1","s_sec2","s_sec3"):
                        try: dpg.set_value(stag, [[],[]])
                        except: pass

            prog = clamp((abs_t - lap_start) / t_span, 0.0, 1.0) * t_span
            dpg.set_value("s_cur", [[prog, prog], [0, 420]])
        except Exception as exc:
            log.warning("update_plot outer: %s", exc, exc_info=True)
    ...
    def _update_analysis(self, snap: Snap, drv: str, abs_t: float):
        try:
            ...
            dpg.set_value("txt_track_cond", f"Track: {ts_txt}")
            dpg.configure_item("txt_track_cond", color=ts_col)

            # safe column getter for lap row
            def col_val(col, default=float("nan")):
                try:
                    if col not in dl.columns: return default
                    v = dl[col].to_numpy(dtype=object)[cli]
                    f = float(v)
            ...
            def col_val_prev(col, default=float("nan")):
                ...
                try:
                    if col not in dl.columns: return default
                    v = dl[col].to_numpy(dtype=object)[cli - 1]
                    f = float(v)
                    return default if math.isnan(f) else f
                except: return default
            ...
            log.debug("analysis setup: %s", e); return

        # ── Sectors (only on lap change) ─────────────────────────────
        try:
                ...
                for si in range(1, 4):
                    ...
                    try:
                        if col2 in dl.columns:
                            sarr = dl[col2].iloc[:cli+1].to_numpy(dtype=float)
                            sval = sarr[~np.isnan(sarr)]
            ...
            log.debug("analysis sectors: %s", e)

        # ── Pace & metrics ───────────────────────────────────────────
        try:
            if "LapTime" not in dl.columns: return
            lt_arr = dl["LapTime"].iloc[:cli+1].to_numpy(dtype=float)
            valid  = lt_arr[~np.isnan(lt_arr)]
            if len(valid) == 0: return

            ll_s      = float(valid[-1])
            delta_str = f"{ll_s - self.session_best_s:+.3f}s" if self.session_best_s else "—"

            if len(valid) >= 2:
                r5   = valid[-min(6, len(valid)):-1]
                avg  = float(np.mean(r5)) if len(r5) else ll_s
                diff = ll_s - avg
            ...
            if len(valid) >= 3:
                ...
                dpg.set_value("pm_deg", "—"); dpg.configure_item("pm_deg", color=TDIM)
            ...
            if cur_snap and not cur_snap.is_out and cur_snap.gap_num < 900:
                ...
                dpg.set_value("pm_gapp1", "—"); dpg.configure_item("pm_gapp1", color=TDIM)

            # Stint avg
            tl = snap.tyre_laps
            stint = valid[-tl:] if 0 < tl <= len(valid) else valid[-5:]
            ...
            if cur_snap:
                ...
                if 0 <= bidx < len(self._last_snaps) - 1:
                    ...
                    if not bs.is_out and bs.int_num < 4.0:
                        ...
                        dpg.set_value("pm_ucut", "Safe"); dpg.configure_item("pm_ucut", color=GREEN)
                else:
                    dpg.set_value("pm_ucut", "—"); dpg.configure_item("pm_ucut", color=TDIM)
            ...
            dpg.set_value("pm_maxspd", f"{int(max_spd)} kph" if max_spd > 0 else "—")
            dpg.configure_item("pm_maxspd", color=CYAN if max_spd > 0 else TDIM)

            # Pit window / laps remaining
            ...
            if snap.tyre_laps > 0 and len(valid) >= 3:
                ...
                dpg.set_value("pm_fc_win", "—"); dpg.configure_item("pm_fc_win", color=TDIM)
            dpg.set_value("pm_fc_gap", f"{laps_rem}L left")
            dpg.set_value("pm_fc_dlt", delta_str)
            ...
            dpg.set_value("pm_cond", ts_n.split(" ",1)[-1][:6] if " " in ts_n else ts_n[:6])
            dpg.configure_item("pm_cond", color=ts_c)
            ...
            log.debug("analysis pace: %s", e)

        # ── Speed metrics from last lap (separate guard) ─────────────
        try:
            tel2  = self.telemetry.get(drv)
            tval2 = self._tel_times.get(drv, np.array([]))
            if tel2 is not None and len(tval2) > 3 and "LapTime" in dl.columns:
                lt_arr2 = dl["LapTime"].to_numpy(dtype=float)
                tsarr   = dl["TimeSec_f"].to_numpy(dtype=float)
                inxt    = int(np.searchsorted(lta, abs_t, "right"))
                if inxt > 0:
                    irow = min(inxt - 1, len(tsarr) - 1)
                    t_end = float(tsarr[irow])
                    lap_t = float(lt_arr2[irow]) if not math.isnan(float(lt_arr2[irow])) else 90.0
                    if not math.isnan(t_end):
                        le   = self.session_start + t_end
                        si2  = int(np.searchsorted(tval2, le - lap_t))
                        ei2  = int(np.searchsorted(tval2, le))
            ...
            log.debug("analysis speed: %s", e)

        # ── Style table (only on new lap) ────────────────────────────
        try:
            completed = dl.iloc[:cli + 1]
            n_c = len(completed)
            if self._ana_last_style_n != n_c or self._ana_last_drv != drv:
                self._ana_last_style_n = n_c
                self._rebuild_style_table(drv, completed)
        except Exception as e:
            log.debug("analysis style: %s", e)

    def _build_narrative(self, snap, valid_laps):
        parts = []; snaps2 = self._last_snaps
        idx = next((i for i,s in enumerate(snaps2) if s.drv==snap.drv), -1)
        if idx < 0: return "—"
        if snap.in_pit: return "IN PIT LANE"
        if snap.is_out: return "OUT / DNF"
        pos = idx + 1
        if pos == 1:
            parts.append("LEADING")
            if len(snaps2)>1 and not snaps2[1].is_out: parts.append(f"+{snaps2[1].int_num:.1f}s")
        else:
            parts.append(f"P{pos}")
            if snap.gap_num < 2: parts.append("DRS range")
            if snap.lap_type == "IN LAP": parts.append("pitting")
            elif snap.pit_out_lap: parts.append("out lap")
        if len(valid_laps) >= 3:
            trend = float(np.polyfit(range(len(valid_laps[-5:])), valid_laps[-5:], 1)[0])
            if trend < -0.1: parts.append("improving")
            elif trend > 0.3: parts.append("degrading")
            else: parts.append("consistent")
        t_raw = snap.tyre.strip("()").upper()
        if t_raw and t_raw not in ("U","","–","-"):
            parts.append(f"{TYRE_FULL.get(t_raw,t_raw)} L{snap.tyre_laps}")
        ovt_n = self._overtake_count.get(snap.drv, 0)
        if ovt_n > 0: parts.append(f"{ovt_n} OVT")
        return "  |  ".join(parts)

    def _rebuild_style_table(self, drv: str, completed: pd.DataFrame):
        """V6.7: Update pre-allocated style rows. ZERO DPG item creation/deletion."""
        # Hide all rows first
        for ri in range(MAX_STYLE_ROWS):
            try: dpg.configure_item(f"srow_{ri}", show=False)
            except: pass

        if "LapTime" not in completed.columns or "TimeSec_f" not in completed.columns:
            return
        ...
        # Show last MAX_STYLE_ROWS laps, newest at bottom
        rows_to_show = list(range(max(0, n_rows - MAX_STYLE_ROWS), n_rows))
        slot = 0
        for ri in rows_to_show:
            try:
                lap_s = float(lt_col[ri]); t_end = float(tsf_col[ri])
                if math.isnan(lap_s) or math.isnan(t_end): continue
                l_st = self.session_start + t_end - lap_s
                l_en = self.session_start + t_end
                si_  = int(np.searchsorted(tel_t, l_st))
                ei_  = int(np.searchsorted(tel_t, l_en))
                n    = ei_ - si_
                if n < 5: continue
                ...
                rpm_c = df["RPM"].to_numpy()[si_:ei_].astype(float) if "RPM" in df.columns else np.zeros(n)

                on_t  = int(np.sum(thr_c > 5))
                wot   = int(np.sum(thr_c >= 98))
                ...
                row_col = TXT
                if p_clip > 3.0: row_col = PRP
                elif self.session_best_s and abs(lap_s - self.session_best_s) < 0.1: row_col = F1_PRP
                elif db and float(db) < -0.3: row_col = F1_GRN
                elif db and float(db) > 1.0:  row_col = RED

                ts_n, _, _ = TRACK_STATUS.get(self._current_ts, TRACK_STATUS["1"])
                cond = ts_n.split(" ", 1)[-1][:5] if " " in ts_n else ts_n[:5]
                vals = (str(ln), fmt_lap(lap_s), db or "—",
                        f"{p_thr:.0f}", f"{p_wot:.0f}", f"{p_brk:.0f}",
                        f"{p_lico:.0f}", f"{p_clip:.0f}", f"{p_rpm:.0f}", cond)

                dpg.configure_item(f"srow_{slot}", show=True)
                ...
                log.debug("style row %d: %s", ri, exc)
    ...
    def _load_calendar(self, year: int):
        ...
        def fetch():
            try:
                ...
                # Queue calendar result for main thread — never call dpg from here
                self._session_q.put_nowait({"ok": False, "err": None,
                                            "_calendar": races, "_year": year})
            except Exception as exc:
                log.warning("Calendar fetch: %s", exc)
        ...
        try:
            ...
            for drv in session.drivers:
                try:
                    ...
                    tp = strip_fastf1(tel)
                    # V6.4: Precompute monotonically-increasing cumulative distance.
                    # "Distance" from FastF1 resets to 0 at each lap start.
                    # We detect resets (drop > 100m) and accumulate an offset.
                    # This removes ALL lap-boundary instability from the timing sort.
                    if "Distance" in tp.columns:
                        raw = tp["Distance"].to_numpy(dtype=float)
                        cumul = np.where(np.isnan(raw), 0.0, raw).copy()
                        offset = 0.0
                        for ki in range(1, len(raw)):
                            prev = raw[ki - 1] if not math.isnan(raw[ki - 1]) else 0.0
                            curr = raw[ki]     if not math.isnan(raw[ki])     else 0.0
                            if curr < prev - 100.0:   # lap reset detected
                                offset += prev
                            cumul[ki] = offset + curr
                        tp["CumulDist"] = cumul
                    else:
                        tp["CumulDist"] = np.full(len(tp), -1.0)
                    t0 = tp["TimeSec"].min(); t1 = tp["TimeSec"].max()
                    if t0 < t_min: t_min = t0
                    if t1 > t_max: t_max = t1
                    telemetry[drv]  = tp
                    tel_times[drv]  = tp["TimeSec"].to_numpy(dtype=float)
                except Exception as exc:
                    log.warning("Driver %s: %s", drv, exc)

            if not telemetry:
                raise ValueError("No telemetry found. Future/cancelled event?")

            # V6.3: Compute real track lap length from telemetry Distance column
            track_lap_len = 5500.0  # default
            for tp in telemetry.values():
                if "Distance" in tp.columns:
            ...
            # V6.6: Official timing arrays per driver — built once at load
            # gap/interval computed from lap-end session times (reliable, no Gap/Interval columns needed)
            off_timing: Dict[str, dict] = {}
            # First pass: collect lap-end absolute times per driver per lap
            # lap_end_abs[lap_n] = {drv: abs_time_when_they_finished_lap_n}
            lap_end_abs: Dict[int, Dict[str, float]] = {}
            ...
            for drv in telemetry:
                try:
                    ...
                    t_arr  = dp["TimeSec_f"].to_numpy(dtype=float) if "TimeSec_f" in dp.columns else np.full(len(dp), np.nan)
                    ln_arr = dp["LapNumber"].to_numpy(dtype=float)  if "LapNumber" in dp.columns else np.arange(1, len(dp)+1, dtype=float)
                    pos_arr= dp["Position"].to_numpy(dtype=float)   if "Position"  in dp.columns else np.full(len(dp), 999.0)
                    lt_arr = dp["LapTime"].to_numpy(dtype=float)    if "LapTime"   in dp.columns else np.full(len(dp), np.nan)
                    ...
                    off_timing[drv] = {
                        ...
                        "lap": ln_arr[valid],
                        "lt":  lt_arr[valid],
                        "gap": np.full(np.sum(valid), np.nan),   # filled in second pass
                        "ivl": np.full(np.sum(valid), np.nan),
                    }
                    # record lap end times for gap computation
                    for i, (ln, t) in enumerate(zip(ln_arr[valid], t_arr[valid])):
                        lni = int(ln) if not math.isnan(ln) else -1
                        if lni > 0:
                            if lni not in lap_end_abs: lap_end_abs[lni] = {}
                            lap_end_abs[lni][drv] = float(t)  # session-relative seconds
                    ...
                    lbd[drv] = pd.DataFrame(); lti[drv] = np.array([])
                    sb[drv] = [None,None,None]
                    off_timing[drv] = {"t":np.array([]),"pos":np.array([]),"lap":np.array([]),"lt":np.array([]),"gap":np.array([]),"ivl":np.array([])}

            # Second pass: compute gap/interval per lap per driver
            # gap  = driver_lap_end_time - earliest_lap_end_time_for_same_lap (= leader's time)
            # ivl  = driver_lap_end_time - previous_position_lap_end_time
            for lap_n, drv_times in lap_end_abs.items():
                sorted_drvs = sorted(drv_times.items(), key=lambda x: x[1])  # sorted by end time
                if not sorted_drvs: continue
                leader_t = sorted_drvs[0][1]
                for rank, (drv, t) in enumerate(sorted_drvs):
                    ot = off_timing.get(drv)
                    if ot is None: continue
                    ln_arr2 = ot["lap"]
                    if len(ln_arr2) == 0: continue
                    # find the index in this driver's arrays for this lap
                    matches = np.where(ln_arr2 == lap_n)[0]
                    if len(matches) == 0: continue
                    idx2 = matches[-1]
                    ot["gap"][idx2] = t - leader_t
                    ot["ivl"][idx2] = t - sorted_drvs[rank-1][1] if rank > 0 else 0.0
            ...
            self.laps_data         = laps
            self._laps_by_drv      = lbd
            self._off_timing       = off_timing
            ...
            self._sector_bests     = sb
            self._overall_sec_best = overall
            self._ers_state        = {}
            self._track_lap_len    = track_lap_len
            self._plot_last_drv    = ""; self._plot_last_lap = -1
            self._strategy_last_hash = ""; self._ers_last_pct = -1.0
            self._timing_slots      = [""] * MAX_DRV
            self._prev_positions    = {}
            # V6.3: reset overtake stats on new session
            self._overtake_count   = {}
            self._pos_lost_count   = {}
            self._overtake_events  = []
            self._last_overtake_str = {}
            self._session_max_speed = {}

            # ── All data ready: post to main thread via _session_q ────
            self._session_q.put({
                "ok": True, "race": race, "year": year, "sess_type": sess_type
            })
        except Exception as exc:
    ...
    def _on_session_loaded(self, race: str, year: int, sess_type: str):
        ...
        try:
            ...
            if fastest is not None and not pd.isna(fastest.get("LapTime")):
                ...
                self._track_x = tel["X"].values
                self._track_y = tel["Y"].values
                self._draw_track()
        except Exception as exc:
    ...
    def _rebuild_timeline(self):
        ...
        for i in range(len(self.track_status_df)):
            ...
            t1 = ((self._ts_times[i+1]-self.session_start)/self.max_time if i+1 < len(self._ts_times) else 1.0)
    ...
    def _draw_track(self):
        dpg.delete_item("dl_map", children_only=True)
        self._map_dots = set()
        if self._track_x is None: return
        W = max(200, dpg.get_item_width("dl_map")  or 200)
        H = max(100, dpg.get_item_height("dl_map") or 100)
        ...
        for i in range(0, n, step):
            sx, sy = self._w2m(self._track_x[i], self._track_y[i], W, H)
            pts.append((sx, sy))
        if pts:
            dpg.draw_polyline(pts, color=_c("40405e"), thickness=4,
                             parent="dl_map", tag="track_poly")
    ...
    def _select_driver(self, drv: str):
        ...
        # Reset all per-driver state so next frame does a full re-render
        self._plot_last_drv    = ""
        self._plot_last_lap    = -1
        self._ana_last_drv     = ""
        self._ana_last_lap     = object()   # force sector/style refresh
        self._ana_last_style_n = -1
        self._strategy_last_hash = ""       # force strategy redraw
        self._ers_last_pct = -1.0           # force ERS bar redraw
        log.info("Selected driver: %s", drv)
        # Force immediate UI update for the new driver
    ...
    def _on_slider(self, sender, value):
        ...
        dpg.set_value("txt_time", hms(int(new_t)))
    ...
    def _refresh_badge(self, sess_type: str):
        si = SESSION_INFO.get(sess_type, (sess_type, TXT))
        dpg.set_value("txt_badge", f"[ {si[0]} ]")
        dpg.configure_item("txt_badge", color=si[1])
